    ...
    try:
        ...
        if not stats["added"] and not stats["dup_skipped"]:
            ...
            try:
                ...
                pass
            if counts:
                # keep the converted CSV + found members; Convert pops a modal
                # picker over the page instead of navigating away (강프로).
                entry = _save_uploaded_csv(user, csv_bytes, csv_name, 0, "")
                items = _load_uploads(user)
                for it in items:
                    if it.get("id") == entry["id"]:
                        it["member_counts"] = dict(counts.most_common(40))
                _save_uploads(user, items)
                return ("redirect",
                        "/cardconv/convert?member_mismatch=" + entry["id"])
...
def _bat_echo(line: str) -> str:
    """One `echo(` line that reproduces `line` verbatim when the bat runs.

    `echo(` rather than `echo `: the space form eats one leading space, which
    would silently re-indent the file being written.

    Inside double quotes cmd stops treating `&` and `>` as special, and stops
    treating `^` as an escape — so escaping there prints the carets themselves
    (measured: a VBS line came out as `2^>^&1`). Percent is the exception; it
    expands regardless of quoting."""
    out, in_quotes = [], False
    for ch in line:
        if ch == '"':
...
def _build_installer() -> bytes:
    """A single self-contained Install SAP Agent.bat.

    The installer needs run-agent.vbs and agent-pair.bat beside it, and telling
    someone to download three files into the same folder is how setup fails. It
    writes its two companions itself, so the whole thing is one file to save and
    double-click — which is also the only shape that travels to another PC."""
    here = _SVC_DIR.parent / "scripts" / "sap_robot"
    body = (here / "install-agent.bat").read_text()
    parts = ["@echo off",
             "REM Generated by Wayfinder - Review > Set up the SAP agent.",
             "REM Writes its two companion files, then runs the setup below.",
             "setlocal", "set HERE=%~dp0"]
    for name in ("run-agent.vbs", "agent-pair.bat"):
        parts.append(f'echo Writing {name}...')
        parts.append(f'> "%HERE%{name}" echo(')          # truncate first
        for ln in (here / name).read_text().splitlines():
            parts.append(f'>> "%HERE%{name}" {_bat_echo(ln)}')
    parts.append("endlocal")
    # The setup itself, minus its own @echo off so the header above stands.
    parts.append(body.split("\n", 1)[1] if body.startswith("@echo off") else body)
    return ("\r\n".join(parts) + "\r\n").encode("ascii")


def _handle_installer(username: str):
    """GET /cardconv/robot/installer — the one file a PC needs."""
    return ("file_inline", _build_installer(),
            "application/octet-stream", "Install SAP Agent.bat")
