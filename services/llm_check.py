...
def render_batch_results():
    ...
    grade_colors = {"A": "#177E42", "B": "#0269A6", "C": "#B45309", "D": "#C62828", "-": "#64748b"}
    ...
    for i, r in enumerate(results):
        ...
        g_color = grade_colors.get(grade, "#64748b")
        bar_color = "#177E42" if pct >= 70 else "#B45309" if pct >= 40 else "#C62828"
        cats = r.get("categories", [])
        cat_scores = " ".join(
            f'<span title="{c["label"]}" style="font-size:0.75rem;color:#64748b">{c["score"]}</span>'
            for c in cats
        ) if cats else ""
        error = r.get("error", "")
        rows += f'''<tr onclick="location.href='/llm-check?url={url}'" style="cursor:pointer">
          ...
          <td style="color:#64748b;font-size:0.75rem">{"⚠ " + error[:40] if error else cat_scores}</td>
        </tr>'''

    summary_bars = "".join(
        f'<div style="text-align:center"><div style="font-size:1.5rem;font-weight:700;color:{grade_colors[g]}">{grade_counts[g]}</div><div style="font-size:0.8rem;color:#64748b">등급 {g}</div></div>'
        for g in ["A","B","C","D"]
    )
...
<div style="padding:32px 40px 40px">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
    <div>
      ...
      <p style="color:#64748b;font-size:0.85rem">{timestamp} · {len(results)}개 URL</p>
    </div>
  </div>
...
def render_form(error=""):
    ...
    if gnb_groups:
        ...
        preset_html = '<div style="color:#64748b;font-size:0.82rem">로딩 중...</div>'
    return f'''<!DOCTYPE html>
<html lang="ko"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
...
<div class="form-wrap">
  <div class="form-card">
    ...
    <div id="recent-section" style="display:none;margin-top:20px;padding-top:20px;border-top:1px solid #f1f5f9">
      <p style="font-size:0.78rem;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:10px">🕐 최근 분석</p>
...
def render_category_panel(current_page_type="other"):
    ...
    page_type_table = (
        ...
        '<span style="color:#177E42;font-weight:700">✓</span> 평가 대상 &nbsp;·&nbsp; '
...
<div style="margin-bottom:24px;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0">
  <div id="guide-header" onclick="toggleGuide()"
    ...
    <div>
      ...
      <span style="margin-left:10px;font-size:0.78rem;color:#64748b">samsung.com/us 운영자 관점 — 각 카테고리가 AI 검색에 미치는 영향</span>
    </div>
...
def render_result(r):
    ...
    strength_items = "".join(
        ...
        f'</div>'
        for c in strengths
    ) or '<span style="font-size:0.8rem;color:#64748b">해당 없음</span>'
    weakness_items = "".join(
        f'<div style="display:flex;align-items:center;gap:6px;padding:3px 0">'
        ...
        f'</div>'
        for c in weaknesses
    ) or '<span style="font-size:0.8rem;color:#64748b">개선 필요 없음</span>'
    summary_html = f'''
    ...
    for idx, c in enumerate(r["categories"]):
        ...
        issues_html = "".join(f'<li style="color:#C62828">⚠ {_html.escape(i)}</li>' for i in c["issues"])
        tips_html_inner = "".join(f'<li style="color:#0269A6">💡 {_html.escape(t)}</li>' for t in c["tips"])
        ...
        if c.get("skipped"):
          ...
          <div style="margin-bottom:10px">
            ...
            <div id="{toggle_id}" style="display:none;margin-top:8px;background:#f8f7ff;border-radius:8px;padding:12px;border-left:3px solid #6d28d9">
              <div style="font-size:0.75rem;color:#64748b;margin-bottom:8px;font-style:italic">이 카테고리가 무엇인지, 왜 평가하는지에 대한 기준입니다. 이 페이지에서 발견된 구체적 문제·조치는 하단 개선 제안을 확인하세요.</div>
    ...
    if not has_any_tip:
        tips_blocks = "<div style='color:#64748b;font-size:0.88rem;padding:16px'>모든 항목 양호 — 개선 사항 없음</div>"
...
<div class="result-layout">
  <div class="result-main">
    ...
    <p style="font-size:0.8rem;color:#64748b;margin:-8px 0 12px">각 항목 우측의 <strong style="color:#6d28d9">📐 평가 루브릭 보기</strong>를 누르면 해당 카테고리의 정의·영향·점수 구간 기준을 확인할 수 있습니다.</p>
    {cat_rows}

    <div class="section-title">💡 카테고리별 개선 제안</div>
    <p style="font-size:0.8rem;color:#64748b;margin:-8px 0 12px">이 페이지 분석에서 실제로 발견된 문제점과 조치 방향입니다. 평가 항목의 정의나 점수 기준이 궁금하면 위 카드의 📐 루브릭을 참고하세요.</p>
